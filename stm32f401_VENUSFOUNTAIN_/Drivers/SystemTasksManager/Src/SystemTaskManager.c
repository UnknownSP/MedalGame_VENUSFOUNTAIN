...
volatile uint8_t PC_control_rcv[8] = {0,0,0,0,0,0,0,0};
static uint8_t PC_control_rcv_data[8] = {0,0,0,0,0,0,0,0};
volatile uint8_t PC_control_temp = 0;
static bool PC_control_flag = true;
...
int main(void){
  ...
  bool user_button_flag = true;
  int button_push_flag = false;
  int count = 0;
...
#if USE_PC_CONTROL
      if(PC_control_flag){
        MW_USART2ReceiveMult(5, PC_control_rcv_data);
        PC_control_flag = false;
      }
      //MW_printf("PC: ");
	    for(int i=0;i<8;i++){
	      //MW_printf("[%3d]",PC_control_rcv_data[i]);
	    }
      //MW_printf("\n");
	    for(int i=0;i<8;i++){
	      PC_control_rcv[i] = PC_control_rcv_data[i];
	    }
#endif
      //MW_printf("\033[1;1H");//カーソルを(1,1)にセットして
      MW_printf("flush %d %d %d %d %d\n",g_SY_system_counter,Pocket_Number,Pocket_In_Number,(int)(Pocket_Number_Detailed * 2.0),button_push_flag);
      //MW_printf("PC: %3d %3d %3d %3d %3d %3d\n",PC_control_rcv_data[0],PC_control_rcv_data[1],PC_control_rcv_data[2],PC_control_rcv_data[3],PC_control_rcv_data[4],PC_control_temp);
      DD_print();//各デバイスハンドラを表示します
      //MW_printf("flush %d %d %d %d\n",g_SY_system_counter,count,100-count,button_push_flag);
      flush(); /* out message. */
    }
    //タイミング待ちを行います
